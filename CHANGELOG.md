...
- **빌드 산출물에 테스트 파일이 섞여 배포되던 것**: `tsconfig` 가 `src/**/*` 를 통째로 컴파일해 `build/lib/dart-xml.test.js` 가 npm 패키지에 실리고, vitest 가 그 컴파일본까지 수집해(픽스처는 복사되지 않으므로) 실패했다. 테스트·픽스처를 빌드 대상에서 제외 (`tsconfig.json`)
...
### Fixed — `download_document` 가 원문 일부를 조용히 흘리던 문제 (#3, @ahnbu)

- **ZIP 내 첨부 XML 누락**: 엔트리를 `files.find()` 로 **첫 XML 하나만** 변환했다. DART 는 첨부문서를 별도 XML 엔트리로 넣기 때문에(감사보고서 + 재무제표 등) 나머지가 조용히 사라지고, 사용자에겐 "원문 XML 에 있는 손익계산서 표가 마크다운 결과에 없다"로 보인다. 모든 XML 엔트리를 변환해 파일명 머리글과 함께 잇고 응답에 `files[]` 를 노출한다. 엔트리가 하나면 종전 출력 그대로 (`src/tools/download-document.ts`)
- **HTML 엔티티가 원문 그대로 노출**: DART 마크업은 HTML 계열이라 `&nbsp;` 로 칸을 맞추는데 XML 파서는 이를 미정의 엔티티로 보고 그대로 둔다 — 실제 공시 1건에서 **332회** 새어나와 표 셀이 `과&nbsp;&nbsp;목` 으로 실렸다. 파싱 전에 푼다 (`src/lib/dart-xml.ts`)

검증: 실제 공시(다원컴퓨팅 감사보고서 `20260406003197`) 본문 마크업 131KB 변환 — 재무상태표·손익계산서·자본변동표·현금흐름표·주석 전부 보존, 엔티티 누출 332 → 0.

### Added — 테스트 인프라

- vitest 도입. 실제 공시 발췌 픽스처 기반 `dart-xml` 회귀 테스트 5건 — 재무제표 표가 통째로 사라지면 CI 에서 잡힌다.
